...
{
    [SessionExpire]
    public class IQFIssuedStockController : Controller
    {
        private readonly ApplicationDbContext db = new ApplicationDbContext();

        // GET: IQFIssuedStock
        [Authorize(Roles = "IQFIssuedStockIndex")]
        public ActionResult Index()
        {
            ViewBag.Title = "IQF Issued Stock";
            return View();
        }

        // GET: IQFIssuedStock/Form/{id}
        [Authorize(Roles = "IQFIssuedStockCreate,IQFIssuedStockEdit")]
        public ActionResult Form(int? id)
        {
            ViewBag.Title = id.HasValue ? "Edit IQF Issued Stock" : "Add IQF Issued Stock";
            ...
            if (id.HasValue && id.Value > 0)
            {
                ...
                var header = db.TransactionMasters
                    .FirstOrDefault(t => t.TRANMID == tranMid && t.REGSTRID == 44);
        ...
        // GET: IQF Issued Stock data for index DataTable (REGSTRID = 44)
        [Authorize(Roles = "IQFIssuedStockIndex")]
        ...
        public JsonResult Save(IssuedStockSaveModel model)
        {
            try
            {
                ...
                if (model.OpeningId.HasValue && model.OpeningId.Value > 0)
                {
                    ...
                    var existing = db.Database.SqlQuery<ExistingIssuedData>(@"
                        ...
                        WHERE TRANMID = @p0 AND REGSTRID = 44",
                        tranMId).FirstOrDefault();

                    if (existing == null)
                    {
                        return Json(new { success = false, message = "IQF Issued Stock record not found for editing." });
                    }
                    ...
                    var updateSql = @"
                        UPDATE TRANSACTIONMASTER SET
                            ...
                            LMUSRID = @p5,
                            PRCSDATE = @p6
                        WHERE TRANMID = @p7 AND REGSTRID = 44";
                ...
                else
                {
                    ...
                    tranMId = db.Database.SqlQuery<int>(insertSql,
                        ...
                        "IQF ISSUED STOCK",    // @p1 CATENAME
                        "IQFISSUED",           // @p2 CATECODE
                        "-",                  // @p3 VECHNO
                        0,                     // @p4 DISPSTATUS
                ...
                {
                    success = true,
                    message = "IQF Issued Stock saved successfully.",
                    tranMId,
                    tranNo,
                    tranDNo
                });
            ...
            {
                return Json(new { success = false, message = "Error saving IQF Issued Stock: " + ex.Message });
            }
        }

        [HttpPost]
        [Authorize(Roles = "IQFIssuedStockDelete")]
        public ActionResult Del(int id)
        {
            try
            {
                var exists = db.Database.SqlQuery<int>(
                    "SELECT COUNT(1) FROM TRANSACTIONMASTER WHERE TRANMID = @p0 AND REGSTRID = 44",
                    id).FirstOrDefault();
                ...
                db.Database.ExecuteSqlCommand(
                    "DELETE FROM TRANSACTIONMASTER WHERE TRANMID = @p0 AND REGSTRID = 44",
                    id);
