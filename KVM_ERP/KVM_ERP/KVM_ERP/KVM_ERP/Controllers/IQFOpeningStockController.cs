...
{
    [SessionExpire]
    public class IQFOpeningStockController : Controller
    {
        private readonly ApplicationDbContext db = new ApplicationDbContext();

        // GET: IQFOpeningStock
        [Authorize(Roles = "IQFOpeningStockIndex")]
        public ActionResult Index()
        {
            ViewBag.Title = "IQF Opening Stock";
            return View();
        }

        // GET: IQFOpeningStock/Form/{id}
        // When id is provided, load existing IQF Opening Stock (REGSTRID = 33)
        // and pass a JSON-ready model for client-side population.
        [Authorize(Roles = "IQFOpeningStockCreate,IQFOpeningStockEdit")]
        public ActionResult Form(int? id)
        {
            ...
            if (id.HasValue && id.Value > 0)
            {
                ...
                var header = db.TransactionMasters
                    .FirstOrDefault(t => t.TRANMID == tranMid && t.REGSTRID == 33);
        ...
        public ActionResult GetAjaxData(string fromDate = null, string toDate = null)
        {
            try
            {
                ...
                // Return ONE row per IQF Opening Stock (TRANMID) for Index view.
        ...
        // JSON: Products by Material Group (enabled only) for IQF Opening Stock form
        [HttpGet]
        ...
        // JSON: Additional filter values for IQF Opening Stock form derived from
        ...
        public JsonResult GetFilterValues(int groupId, string asOnDate = null)
        {
            try
            {
                ...
                // No of Slabs: use per-pack No of Slabs/Boxes (PCKBOX)
        ...
        public JsonResult Save(OpeningStockSaveModel model)
        {
            try
            {
                ...
                if (model.OpeningId.HasValue && model.OpeningId.Value > 0)
                {
                    ...
                    var existing = db.Database.SqlQuery<ExistingOpeningData>(@"
                        ...
                        WHERE TRANMID = @p0 AND REGSTRID = 33",
                        tranMId).FirstOrDefault();

                    if (existing == null)
                    {
                        return Json(new { success = false, message = "IQF Opening Stock record not found for editing." });
                    }
                    ...
                    var updateSql = @"
                        UPDATE TRANSACTIONMASTER SET
                            ...
                            LMUSRID = @p6,
                            PRCSDATE = @p7
                        WHERE TRANMID = @p8 AND REGSTRID = 33";
                ...
                else
                {
                    ...
                    tranMId = db.Database.SqlQuery<int>(insertSql,
                        ...
                        "IQF OPENING STOCK",   // @p1 CATENAME
                        "IQFOPENING",         // @p2 CATECODE
                        "-",                   // @p3 VECHNO
                ...
                {
                    success = true,
                    message = "IQF Opening Stock saved successfully.",
                    tranMId,
                    tranNo,
                    tranDNo
                });
            ...
            {
                return Json(new { success = false, message = "Error saving IQF Opening Stock: " + ex.Message });
            }
        }
        ...
        public ActionResult Del(int id)
        {
            try
            {
                if (!User.IsInRole("IQFOpeningStockDelete"))
                {
                    return Json("Access Denied: You do not have permission to delete IQF Opening Stock records. Please contact your administrator.");
                }

                var exists = db.Database.SqlQuery<int>(
                    "SELECT COUNT(1) FROM TRANSACTIONMASTER WHERE TRANMID = @p0 AND REGSTRID = 33",
                    id).FirstOrDefault();
                ...
                db.Database.ExecuteSqlCommand(
                    "DELETE FROM TRANSACTIONMASTER WHERE TRANMID = @p0 AND REGSTRID = 33",
                    id);
